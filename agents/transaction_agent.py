...
class TransactionAgent:
    ...
    def run(self, bundle: DatasetBundle) -> pd.DataFrame:
        ...
        sender_directory = directory.rename(
            columns={"actor_id": "sender_id", "salary": "sender_salary", "age": "sender_age"}
        )
        ...
        merged["has_recipient_iban"] = merged["recipient_iban"].fillna("").ne("").astype(int)
        ...
        merged["amount_to_salary"] = merged["amount"] / (merged["sender_salary"].fillna(0) + 1.0)
        merged["is_round_amount"] = (merged["amount"] % 10 == 0).astype(int)
        merged["description_risk"] = merged["description"].fillna("").map(self._description_risk)
        merged["transfer_without_iban"] = (
            (merged["transaction_type"] == "transfer") & (merged["has_sender_iban"] == 0)
        ).astype(int)
        merged["inperson_without_location"] = (
            (merged["transaction_type"] == "in-person payment") & (merged["has_location"] == 0)
        ).astype(int)
        ...
        merged["sender_amount_deviation"] = merged["sender_amount_deviation"].fillna(0.0)
    ...
    def _build_sender_history_features(self, tx: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, group in tx.groupby("sender_id", sort=False):
            ...
            for row in group.sort_values("timestamp").itertuples(index=False):
                ...
                while recent_window and (current_ts - recent_window[0]).total_seconds() > 24 * 3600:
                    recent_window.popleft()

                prev_count = len(history_amounts)
                median_amount = float(np.median(history_amounts)) if history_amounts else 0.0
                mad_amount = (
                ...
                if history_amounts and mad_amount > 0:
                    amount_deviation = float(abs(row.amount - median_amount) / (mad_amount + 1.0))
                elif history_amounts and median_amount > 0:
                    amount_deviation = float(abs(row.amount - median_amount) / (median_amount + 1.0))

                recipient_is_new = 1 if row.recipient_id not in seen_recipients else 0
                type_rarity = 1.0 if prev_count == 0 else 1.0 - (type_counter[row.transaction_type] / prev_count)
                method_value = row.payment_method or "<empty>"
                method_rarity = (
                    1.0 if prev_count == 0 else 1.0 - (method_counter[method_value] / prev_count)
                )
